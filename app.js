(() => {
  ...
  async function checkConnection(){const el=$('connectionState');el.textContent='연결 확인 중...';el.classList.remove('ok');try{const d=await api({action:'status'});el.textContent=(d.username&&String(d.username).toUpperCase()!=='API_CONNECTED')?`✅ Instagram 계정 연결 완료 · @${d.username}`:'✅ Instagram API 연결 확인됨';el.classList.add('ok')}catch(e){el.textContent='연결 안 됨 · '+e.message}}
  function runComparison(commenterNames,sourceLabel){
    ...
    $('statParticipants').textContent=parsed.items.length;$('statCommented').textContent=commented.length;$('statMissing').textContent=missing.length;$('statExcluded').textContent=excludedItems.length;$('statFreePass').textContent=freePassItems.length;$('missingTitleCount').textContent=missing.length+'명';$('checkedAt').textContent=new Date().toLocaleString('ko-KR');$('checkedPost').textContent=sourceLabel||$('postUrl').value.trim();
    $('missingList').innerHTML=!missing.length?'<div class="ok-message">🎉 누락자가 없습니다.</div>':missing.map(x=>`<div class="missing-row"><div>${x.no?`<small>${x.no}. ${esc(x.nickname||'')}</small><br>`:(x.nickname?`<small>${esc(x.nickname)}</small><br>`:'')}<strong>@${esc(x.username)}</strong></div></div>`).join('');$('resultCard').classList.remove('hidden');$('resultCard').scrollIntoView({behavior:'smooth',block:'start'});
  }
