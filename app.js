...
@rabbit_mom`,
  ...
  // ===== 회원별 Instagram 연결 =====
  const MEMBER_CLIENT_STORAGE_KEY='yeowoobang_member_client_id_v1';
  const MEMBER_USERNAME_STORAGE_KEY='yeowoobang_member_instagram_username_v1';

  function getMemberClientId(){
    let value='';
    try{ value=String(localStorage.getItem(MEMBER_CLIENT_STORAGE_KEY)||'').trim(); }catch(e){}
    if(/^[A-Za-z0-9._:-]{8,120}$/.test(value)) return value;

    const randomPart=(window.crypto&&crypto.randomUUID)
      ? crypto.randomUUID()
      : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`;
    value=`yb-${randomPart}`.replace(/[^A-Za-z0-9._:-]/g,'').slice(0,120);
    try{ localStorage.setItem(MEMBER_CLIENT_STORAGE_KEY,value); }catch(e){}
    return value;
  }

  function saveMemberUsername(username){
    const id=normalizeIg(username);
    if(!id) return '';
    try{ localStorage.setItem(MEMBER_USERNAME_STORAGE_KEY,id); }catch(e){}
    if($('ownerId')){
      $('ownerId').value=id;
      try{ localStorage.setItem('yeowoobang_instagram_owner',id); }catch(e){}
    }
    return id;
  }

  function clearMemberUsername(){
    try{ localStorage.removeItem(MEMBER_USERNAME_STORAGE_KEY); }catch(e){}
  }

  function setMemberIgUi(data){
    ...
    if(connected){
      const username=saveMemberUsername(data.username||'');
      if(state) state.textContent=username?`@${username}`:'연결됨';
      if(help) help.textContent='연결 완료 · 이 계정의 게시물 댓글을 자동 확인할 수 있어요.';
      if(connect) connect.textContent='Instagram 재연결';
      disconnect?.classList.remove('hidden');
    }else{
      if(state) state.textContent='연결된 계정 없음';
      if(help) help.textContent='본인의 프로페셔널 Instagram 계정을 먼저 연결해주세요.';
  ...
  async function startMemberInstagramConnect(){
  ...
  async function disconnectMemberInstagram(){
    const btn=$('memberIgDisconnectBtn');
    try{
      if(btn) btn.disabled=true;
      const data=await backendApi({action:'memberDisconnect',clientId:getMemberClientId()});
      if(data&&data.ok===false) throw new Error(data.message||data.error||'연결 해제 실패');
      clearMemberUsername();
      setMemberIgUi({connected:false});
      if($('hybridApiStatus')){
        $('hybridApiStatus').className='hybrid-status';
        $('hybridApiStatus').textContent='Instagram 연결을 해제했습니다.';
      }
    }catch(e){
      alert('Instagram 연결 해제 실패: '+String(e.message||e));
    }finally{
  ...
  function handleMemberOAuthReturn(){
    ...
    if(params.get('member_oauth')!=='success') return false;
    const username=saveMemberUsername(params.get('username')||'');
    params.delete('member_oauth');
    params.delete('username');
    const query=params.toString();
    try{ history.replaceState({},document.title,location.pathname+(query?'?'+query:'')+location.hash); }catch(e){}
  ...
  async function runHybridApiCheck(){
    ...
    try{
      const auth=await backendApi({action:'memberAuthStatus',clientId:getMemberClientId()});
      if(auth&&auth.ok===false) throw new Error(auth.message||auth.error||'Instagram 연결 상태 확인 실패');
      if(!auth?.connected) throw new Error('먼저 본인의 Instagram 계정을 연결해주세요.');
      saveMemberUsername(auth.username||'');

      const d=await backendApi({action:'memberComments',clientId:getMemberClientId(),postUrl});
      if(d&&d.ok===false) throw new Error(d.message||d.error||'댓글 자동 확인 실패');

      const comments=Array.isArray(d.comments)?d.comments:[];
      const names=[...new Set(comments.map(c=>normalizeIg(c?.username)).filter(Boolean))];
      const fetchedCount=Number(d.commentCount||comments.length||0);
      const instagramCount=Number(d.instagramCommentCount||0);

      if(instagramCount>0 && fetchedCount===0){
        throw new Error(`Instagram에는 댓글 ${instagramCount}개가 있지만 API에서 댓글 목록을 가져오지 못했습니다. 잘못된 누락 판정을 막기 위해 검사를 중단했습니다.`);
      }

      status.className='hybrid-status success';
      status.textContent=`자동 조회 성공 · 댓글 ${fetchedCount}개 · 작성자 ${names.length}명 확인`;
      runComparison(names,`내 Instagram 게시물 API 자동 확인 · ${postUrl}`);
      return;
    }catch(e){
  ...
  function renderFileList(){
    ...
    if(count) count.textContent=String(selectedFiles.length);
    ...
    box.innerHTML=selectedFiles.map((file,index)=>{
      const mb=(Number(file.size||0)/1024/1024).toFixed(1);
      return `<div class="file-chip"><span>${index+1}. ${esc(file.name||'캡처')}</span><small>${mb} MB</small></div>`;
    }).join('');
  }
