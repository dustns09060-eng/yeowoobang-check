...
async function bootstrapAuth() {
  ...
  try {
    ...
    // 같은 탭에서 Instagram OAuth를 다녀온 경우에는
    // 직전에 통과했던 여우방 접속 인증을 이어서 사용합니다.
    if (
      oauthReturned &&
      resume &&
      resume.accessGranted &&
      Date.now() - Number(resume.ts || 0) <= 10 * 60 * 1000
    ) {
      accessGranted = true;
      adminLoggedIn = false;
      adminPasswordValue = "";
...
const PUMASI_OAUTH_RETURN_SESSION_KEY = "yeowoobang:pumasiOauthReturn:v1";
...
async function refreshPumasiAuthStatus(showToast = false) {
  ...
  try {
    const data = await pumasiApiGet("memberAuthStatus", {
...
function readPumasiOAuthReturnSession() {
  try {
    const raw = sessionStorage.getItem(PUMASI_OAUTH_RETURN_SESSION_KEY);
    if (!raw) return null;
    ...
    if (!Number.isFinite(age) || age < 0 || age > 10 * 60 * 1000) {
      sessionStorage.removeItem(PUMASI_OAUTH_RETURN_SESSION_KEY);
      return null;
    }
...
function clearPumasiOAuthReturnSession() {
  try { sessionStorage.removeItem(PUMASI_OAUTH_RETURN_SESSION_KEY); } catch (_) {}
}
...
function initPumasiStage1() {
  ...
  $("pumasiConnectBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      const data = await pumasiApiGet("memberAuthStart", { clientId: getPumasiClientId() });
  ...
  $("pumasiApiBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      const auth = await pumasiApiGet("memberAuthStatus", { clientId: getPumasiClientId() });
      if (!auth.connected) throw new Error("먼저 Instagram 계정을 연결해 주세요.");
      savePumasiConnectedUsername(auth.username);

      const data = await pumasiApiGet("memberComments", {
        clientId: getPumasiClientId(),
        postUrl,
      });
